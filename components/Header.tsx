...
type Props = {
  dark: boolean;
  setDark: (v: boolean | ((s: boolean) => boolean)) => void;
};

const Header: FC<Props> = ({ dark, setDark }) => {
  return (
    <View className="flex-row justify-between items-center mb-3 w-[327px] md:w-[540px] top-12 md:top-24">
      <View>
        <Text className="text-white text-xl md:text-4xl uppercase font-bold tracking-[15px]">todo</Text>
      </View>
      <View className="flex-row items-center">
        <TouchableOpacity onPress={() => setDark((s) => !s)} accessibilityRole="button" accessibilityLabel="Toggle theme">
          <SvgXml xml={dark ? darkIcon : lightIcon} width={26} height={26} />
        </TouchableOpacity>
      </View>
    </View>
  );
